...
export function buildExperienceReaderHtml({ books = [], chapters = [], selectedBookId = '', selectedChapterId = '', speed = 1, completed = false, progressText = '', activeCellIndex = null } = {}) {
  ...
  const segments = selectedChapter?.segments?.length ? selectedChapter.segments : [{ title: selectedChapter?.title || '', text: selectedChapter?.text || '', brailleCells: cells }]
  let cellOffset = 0
  const textHtml = segments.map(segment => `<p data-segment-id="${esc(segment.passageId || segment.title || '')}">${esc(segment.text)}</p>`).join('')
  const cellHtml = segments.map(segment => {
    const html = segment.brailleCells.map((cell, index) => {
      const absoluteIndex = cellOffset + index
      return `<span class="braille-playback-cell${absoluteIndex === activeCellIndex ? ' is-playing' : ''}" data-cell-index="${absoluteIndex}" aria-label="盲文第 ${absoluteIndex + 1} 方">${esc(dotsToUnicode(cell))}</span>`
    }).join('')
