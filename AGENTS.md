...
- [x] **v9 全量重构完成**：四入口应用壳（首页/学习盲文/AudioBraille 实验/笔记），听书归入实验；学习四大区域→细项→单项自由跳转；进度经 storage.js 持久化；描述式拼音 TTS；统一逐方输入；亮色教育风 UI；旧 pageViews/bind 接线移除
- [x] v9 验证：35 个测试文件、242 个测试通过；生产构建成功；Playwright 4174 验证启动门、四入口、学习点阵、实验双场景、笔记双朗读、语言切换和移动端无横向溢出
- [x] v10 拼音对照显示：完整音节显示带调拼音，不完整但可识别的方显示声母/韵母/阴平等结构描述，未知方显示 `·`
- [x] v11 textarea 单一盲文文档视图：笔记/听书使用原生 textarea 光标，支持直接编辑、Enter 换行和自动换行；移除重复文档方块
- [x] v11 JSON 笔记备份保留 `documentCells`：重新导入后恢复多方编辑边界，兼容旧 JSON 文本导入
- [x] v12 完整基础拼音音节课程：391 项基础音节、单项声调选择、复杂韵母覆盖和旧进度兼容
- [x] v12 多方键位提示使用 `*` 分隔，并修复学习 AudioBraille 与 TTS 同时播放
- [x] **v13 Task 1–6**：研究协议、版本化辨识题库、听书材料库、本地正式记录/离线队列、Vercel API 与 Supabase schema 已完成
- [x] **v13 Task 7**：正式实验接线完成；同意/背景/统一训练/校准、四类正式辨识、三段固定听书、匿名批量上传及 JSON/CSV 导出已完成
- [x] **v13 Task 8**：实验数据字典、后期分析说明、全量测试/构建/语法检查和浏览器 casual/formal 隔离走查已完成
