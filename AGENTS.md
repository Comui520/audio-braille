...
- [x] 用户试用反馈修复：小键盘点位重复按键切换（再次按下取消点位）；教学“考”阶段保留原版面高度并用“查看”遮罩答案，支持恢复参考信息
- [x] 后续需求实现：训练阶段真实输入显示与播放门控；独立体验听书、多书章节和明文/盲文片段对照；逐方播放高亮；体验/正式听书状态隔离；暂停次数按播放重置；实验标签切换清空输入；启动门避让 Ctrl/Alt/Meta 快捷键；49 个测试文件、339 个测试通过，生产构建成功
- [x] 入口与流程边界调整：顶部导航加入体验盲文听书；AudioBraille 实验仅保留听觉辨识；正式听书作为正式辨识后的最后一节；修复正式实验状态不阻塞独立体验听书
- [x] 任务 11：部署验证（Vercel）——用户已完成 GitHub → Vercel 部署，网站可用
- [x] Supabase 基础接入完成：安全 schema、Vercel API、SUPABASE_SECRET_KEY/旧变量兼容、.env 忽略规则与示例配置已完成；线上环境变量和真实线上写入仍需用户在 Vercel 中配置后验证
